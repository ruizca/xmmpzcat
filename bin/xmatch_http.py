...
def login():
    """
    login to the xmatch server.
    """
    while True:
        cmd = './xmatch.sh i'
        sh_return = sp.check_output(cmd, shell=True)
...
def rmfile(filename):
    ...
    """
    cmd = './xmatch.sh r {}'.format(filename)
    sp.call(cmd, shell=True)
...
def addfile(filename):
    ...
    """
    while True:
        cmd = './xmatch.sh p {}'.format(filename)
        sh_return = sp.check_output(cmd, shell=True)
...
def getfile(filename):
    ...
    """
    cmd = './xmatch.sh g {}'.format(filename)
    sp.call(cmd, shell=True)


def makeXmsFile(catalogues, prefixes, completeness):
    """
    Prepare an xms file with the data needed to cross-match 'catalogues'.
    """
    ...
    with open('temp.xms', 'w+') as xmsfile:
        xmsfile.write(xms_cmd)
