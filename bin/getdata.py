...
def xmm(obsids_table, data_folder, src_filename, nir_moc=None, 
        moc_order=16, radius=15, use_poscorr=False) :
    """
    Get XMM sources
    ...
    if nir_moc is None :
        ...
        root, ext = os.path.splitext(os.path.basename(nir_moc))
    ...
    for i, row in enumerate(tqdm(obsids_table, desc="Making XMM groups")) :
        ...
        if area_field[i] == 0 :
            ...
            msk_src = src_catalog.separation(field_coords) <= radius*u.arcmin
...
def tmass(obsids_table, data_folder, moc_folder, 
    ...
    for i, row in enumerate(tqdm(obsids_table, desc="Making 2MASS groups")) :
        ...
        if overwrite or (not overwrite and not is_field_table) :
            ...
            vrsp = v.query_region_async(field_coords, radius=radius, 
                                        catalog='II/246/out', return_type='asu-tsv')
...
def ukidss(obsids_table, data_folder, moc_folder, nir_moc=None, 
         radius=15*u.arcmin, moc_order=16, overwrite=True) :
    """
...
def vista(obsids_table, data_folder, moc_folder, nir_moc=None, 
         radius=15*u.arcmin, moc_order=16, overwrite=True) :
    """
